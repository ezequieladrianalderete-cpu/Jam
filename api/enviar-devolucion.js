  ...
  try {
    ...
    for (const p of puntajes) {
      ...
      const itemsStr = Object.entries(items)
        ...
        .join(" · ");
      tablaRows += `<tr>
        <td style="padding:10px 14px;border-bottom:1px solid #222;color:#C9A84C;font-weight:700">Jurado ${p.juez_num}</td>
    ...
    const attachments = [];
    for (const p of puntajes) {
      if (p.audio_url && p.audio_url.startsWith("data:")) {
